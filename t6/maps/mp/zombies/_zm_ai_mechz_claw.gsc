...
claw_grapple()
{
    ...
    n_time = n_dist / level.panzer_hook_speed;
    ...
    if ( isdefined( self.e_grabbed ) )
    {
        n_time = n_dist / level.panzer_hook_good_retract_time;
        if (isdefined(level.panzer_faster_pull) && level.panzer_faster_pull == 1)
            wait 0.1;
        else
            wait 0.5;
    }
    ...
    {
        n_time = n_dist / level.panzer_hook_bad_retract_time;
        wait 0.5;
    }
...
should_do_claw_attack()
{
    ...
    if ( n_dist_sq < level.panzer_hook_min_range || n_dist_sq > level.panzer_hook_max_range ) //original is 90k-1M dist
    {
