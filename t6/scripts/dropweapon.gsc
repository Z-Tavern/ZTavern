...
dropPlayerWeapon()
{
    ...
    for(;;)
    {
    	if (getDvar("drop") != "0")
        {
            ...
            if (!getPlayerByGuid(drop[0]) maps\mp\zombies\_zm_laststand::player_is_in_laststand() && getPlayerByGuid(drop[0]) getcurrentweapon() != "syrette_zm" && getPlayerByGuid(drop[0]) getcurrentweapon() != "syrette_afterlife_zm" && getPlayerByGuid(drop[0]) getcurrentweapon() != "zombie_tomahawk_flourish")
            {
...
#/
    ...
    if (weapon == "syrette_zm" || weapon == "syrette_afterlife_zm" )
    {
        return;
    }
    if ( weapon == "none" )
