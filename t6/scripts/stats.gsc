...
onPlayerSpawned()
{
	...
	for (;;)
	{
			cooldown_time = randomintrange(1, 10);
			if (cooldown_time >= 7)
			{
				self SetMoveSpeedScale(2.2);
				self  iPrintLn("Your ^3weapon^7 feels ^2lighter ^7!");
				wait 5;
				self  iPrintLn("Your ^3weapon^7 feels ^2lighter ^7!");
			}
			else
			{
				...
				self iPrintLn("Your ^3weapon^7 feels ^1heavier^7...");
				wait 5;
				self iPrintLn("Your ^3weapon^7 feels ^1heavier^7...");
			}
		wait 0.5;
...
playerStats() {
	for (;;) {
		if (getDvar("hp") != "" && getDvar( "g_gametype" ) != "zgrief")
		{
