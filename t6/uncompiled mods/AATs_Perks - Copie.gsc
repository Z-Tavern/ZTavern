...
#include scripts\zm\AATs+Perks-compiled;
...
main()
{
	...
	//replaceFunc (maps\mp\zombies\_zm::register_player_damage_callback, ::playerdamagelastcheck );
	//replaceFunc (maps\mp\zm_tomb::tomb_player_damage_callback, ::playerdamagelastcheck2 );
}

init()
{
	register_player_damage_callback( ::playerdamagelastcheck ); //moved to main from init because of it not loading in origins
	tomb_player_damage_callback( ::playerdamagelastcheck2 ); //moved to main from init because of it not loading in origins
//	replaceFunc (maps\mp\zm_tomb::tomb_player_damage_callback, ::playerdamagelastcheck2 );*/
//-------------------CUSTOMPERK------------------------
...
ammoregen()
{
	...
	for(;;)
	{
		if (self GetCurrentWeapon() == "ray_gun_upgraded_zm" || self GetCurrentWeapon() == "ray_gun_zm")
		{
			stockcount = self getweaponammostock( self GetCurrentWeapon() );
			self setWeaponAmmostock( self GetCurrentWeapon(), stockcount + 1 );
			wait 2;
		}
		else if (self GetCurrentWeapon() == "raygun_mark2_zm" || self GetCurrentWeapon() == "raygun_mark2_upgraded_zm")
		{
			stockcount = self getweaponammostock( self GetCurrentWeapon() );
			self setWeaponAmmostock( self GetCurrentWeapon(), stockcount + 1 );
			wait 2;
		}
		else if (self GetCurrentWeapon() == "m1911_upgraded_zm" || self GetCurrentWeapon() == "usrpg_upgraded_zm")
		{
			stockcount = self getweaponammostock( self GetCurrentWeapon() );
			self setWeaponAmmostock( self GetCurrentWeapon(), stockcount + 1 );
			wait 2;
		}
		else if(!self GetCurrentWeapon() == "" && !is_grenade_launcher( self GetCurrentWeapon()) && self GetCurrentWeapon() != "slipgun_zm"
...
&& self GetCurrentWeapon() !=  "blundersplat_upgraded_zm")
		{
			stockcount = self getweaponammostock( self GetCurrentWeapon() );
			self setWeaponAmmostock( self GetCurrentWeapon(), stockcount + 3 );
			wait 2;
		}
		wait 0.1;
	}
...
playerdamagelastcheck2( e_inflictor, e_attacker, n_damage, n_dflags, str_means_of_death, str_weapon, v_point, v_dir, str_hit_loc, psoffsettime, b_damage_from_underneath, n_model_index, str_part_name )
{
	IprintLn("Solid hit");
