...
{   
   ...
   // replaceFunc(maps\mp\zombies\_zm::init_levelvars, ::init_levelvars);
    ...
    wait 3.5;
    iPrintLn("^3[ ^1" + level.boss.name + " ^7] : ^3Let us see if you are up for the challenge");


    for (;;)//after killing boss effects and sound
    {
        if (level.boss.health <= 0 )
        {
            playsoundatposition( "mus_zombie_game_over_ee", ( 0, 0, 0 ) );
            iPrintLn("^3[ ^1" + level.boss.name + " ^7] : ^3Well done, you lived up to your reputation");
            foreach(player in level.players)
            {
            ...
            for(i = 0; i <= 1000; i++) // find all zombies in the zombie array and kill
            {
                ...
                zombies[0] dodamage_wrapper(zombies[0].health + 100);
                wait 0.05;
            }
            break;
        }
...
boss_think_Ivo1() // After boss spawned, start the attacks rotation
{
    ...
    wait 7; // let the boss time to get to the middle
...
IvoPhase1_H1_Ivo1()//60
{
    ...
    iPrintLn("^3[ ^1" + level.boss.name + " ^7] : ^3COME FOURTH INFERNOS!");
    wait 3.5;
    iPrintLn("^3[ ^1" + level.boss.name + " ^7] : ^3Let's up the voltage ^1:)");// Titb Boss reference
    wait 22;
    iPrintLn("^3[ ^1" + level.boss.name + " ^7] : ^3Helmets ON lads");// Botb Boss Reference
...
Inferno_BURN2_Ivo1()
{
    if (distance(level.inferno2.origin, self.origin) <= 150) //if close to boss //DamFunc
        {
            self dodamage_wrapper(600); // deal damage to self (you gotta run away !) //DamFunc
            vsmgr_activate( "overlay", "zm_transit_burn", self, 1, 1 );//visual burn effect      
            wait 0.05;
...
Phase_check1_Ivo1()
{
    for (;;)
    {
        if (level.boss.health <= level.boss.maxhealth * 0.33)//33% HP
        {
            ...
            level.boss.candamage = 0;
            level.phase = 1;
            iPrintLn("^3[ ^1" + level.boss.name + " ^7] : ^3Let's see how you handle this !");// Botb Boss Reference
            break;
        } 
        wait 0.05;
    }
