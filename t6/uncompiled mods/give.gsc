...
giveCommand()
{
	...
	for (;;)
    {
		if (getDvar("give") != "")
        {
            ...
            {
                weapon_list = level.players[int(content[0])] GetWeaponsListPrimaries();
                level.players[int(content[0])] TakeWeapon(weapon_list[int(content[2])]);         
                level.players[int(content[0])] GiveWeapon(content[1], int(content[2]));
                level.players[int(content[0])] SwitchToWeapon(content[1]);
                level.players[int(content[0])] IPrintLn("You have been given ^3" + content[1] + "^7");
            }
		}
		wait 0.5;
	}	
